...
const AccessDenied = ({ userRole, requiredRole }: { userRole: string | null; requiredRole: string }) => (
...
const RequireAuth = ({ children, requiredRole }: RequireAuthProps) => {
    ...
    const fetchUserRole = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setAccessDenied(true);  // Fail-safe if there's an error fetching the role
          return;
        }
        ...
        setAccessDenied(true);  // Fail-safe if there's a fetch error
      }
    };
  ...
  // Check if user has the required role, and if not, deny access
  useEffect(() => {
    if (!isInitialized || !user || !requiredRole || userRole === null) return;
    ...
    setAccessDenied(!hasRequiredRole);
  }, [isInitialized, user, requiredRole, userRole]);

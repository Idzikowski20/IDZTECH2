...
const Navbar = () => {
  ...
  return <nav className={cn("fixed top-0 w-full z-50 transition-all duration-300", scrolled ? "bg-premium-dark/90 backdrop-blur-sm py-4" : "bg-transparent py-6")}>
      <div className="container mx-auto px-4 lg:px-8 flex justify-between items-center">
        ...
        <div className="hidden lg:flex items-center space-x-8">
          {menuItems.map((item, index) => <div key={index} className="relative">
              {item.submenu ? <button ref={servicesButtonRef} onClick={toggleServicesMenu} className="text-premium-light/80 hover:text-premium-light transition-colors flex items-center gap-1">
                  {item.label}
                  <ChevronDown size={16} className={cn("transition-transform", isServicesOpen ? "rotate-180" : "")} />
                </button> : item.href.startsWith('/') ? <Link to={item.href} className="text-premium-light/80 hover:text-premium-light transition-colors">
                    {item.label}
      ...
      {isServicesOpen && <div className="fixed inset-0 z-40">
          <div className="absolute inset-0 bg-premium-dark/80 backdrop-blur-md"></div>
          <div ref={servicesRef} onClick={e => e.stopPropagation()} className="fixed inset-0 flex items-start justify-center pt-24">
            <div className="bg-premium-dark/95 border border-premium-light/10 rounded-lg p-12 w-full max-w-5xl grid grid-cols-4 gap-10 transform transition-all duration-300 animate-fade-in">
              {servicesCategories.map((category, index) => <div key={index} className="flex flex-col">
                  ...
                  <div className="flex flex-col space-y-3">
                    {category.links.map((link, linkIndex) => <Link key={linkIndex} to={link.href} onClick={() => setIsServicesOpen(false)} className="text-premium-light/70 hover:text-premium-blue transition-colors text-sm group relative overflow-hidden">
      ...
      {/* Mobile Navigation */}
      <div className={cn("lg:hidden fixed inset-0 bg-premium-dark/98 backdrop-blur-sm z-40", isOpen ? "flex flex-col justify-center items-center opacity-100 pointer-events-auto transition-opacity duration-300 ease-in-out" : "opacity-0 pointer-events-none transition-opacity duration-300 ease-in-out")}>
